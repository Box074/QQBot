import { getModDate } from "./modinfo.js";
export function processingModName(name) {
    return name.trim().toLowerCase().replaceAll(' ', '');
}
export function getModRepo(repo) {
...
export function prepareFilter(input, customFilter, aliasGetter) {
    if (!input)
        return [];
    input = input.trim();
    const filters = [];
        ...
        if (p[0] != ':' || filterName == 'name') {
            ...
            filters.push(['author', (mod) => {
                    if (!authorName)
                        return [true, 0];
                    if (mod.authors) {
                        if (mod.authors.map(x => x.toLowerCase()).includes(authorName))
                            return [true, 0];
                    }
                    if (!mod.repository)
                        return [false, 0];
...
export function filterMods(inmods, filters, convert = (mod) => mod) {
    const result = [];
